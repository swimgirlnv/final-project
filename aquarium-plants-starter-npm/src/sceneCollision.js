...
export function findValidPosition(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const x = xRange[0] + margin + Math.random() * (xRange[1] - xRange[0] - 2 * margin);
    const z = zRange[0] + margin + Math.random() * (zRange[1] - zRange[0] - 2 * margin);
    
    if (isInsideTank(x, z, margin) && !checkCollision2D(x, z, radius, minSeparation)) {
      return { x, z };
    }
  }
